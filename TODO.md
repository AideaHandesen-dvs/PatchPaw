...
- **Controller 直接テスト追加**: `tests/test_controller.py` 新設 (12 ケース)。
  iteration ループ、validation/dry_run/test 失敗での retry、max_iterations
  達成、ユーザー拒否、LLM 例外、トークン/elapsed/patch_files 累積仕様、
  carry-context 伝達、`.patchpaw/context.md` 注入を網羅。LLMAdapter と
  TestRunner だけスタブ化、他の依存は本物のまま。バグ発見なし
  (= 既存挙動はすべて期待通り、今後の回帰起点として機能)。
  罠 4.8 の方針 (直接テスト密度の偏在を埋める) に従う — 2026-05-30
