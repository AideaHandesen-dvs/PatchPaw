...
- Request SEARCH/REPLACE or SEARCH_ALL/REPLACE_ALL block output only
- SEARCH/REPLACE: unique single-location change (count == 1 を要求、
  行ブロック単位の規約)
- SEARCH_ALL/REPLACE_ALL: 同一リテラル部分文字列を全箇所置換
  (count >= 1)。リネームや定数置換のように同一文字列が複数箇所に出る
  ケース用。正規表現ではなくリテラル一致。ブロック区切りの末尾改行は
  パーサが trim する (識別子マッチが行末まで含まれないように)
...
- Apply patches via literal string replacement with automatic rollback on
  failure (rollback は `originals` dict によるファイル単位の全文保存方式、
  両モード共通)
...
Checks generated blocks (SEARCH/REPLACE と SEARCH_ALL/REPLACE_ALL の両方)
for format errors, dangerous patterns, and out-of-scope file paths.
...
- `llm_elapsed_total_s`: run 全体の LLM 呼び出し時間合計 (秒)。
  `total_duration_s` (タスク全体の wall-clock) との差が、テスト実行や承認
  待ちで使われた時間
...
- `tasks[]`: 各タスクの `task`, `success`, `duration_s`, `llm_elapsed_s`,
  ...
  `patch_files` (repo_root 相対の patch パス、iteration 順)
